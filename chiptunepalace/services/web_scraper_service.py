...
class WebScraperService:
    ...
    def _get_vgmrips_packs(self, console_url: str) -> list:
        ...
        results = []
        max_pages = 8  # Scrape up to 8 pages to get a massive catalog of 160+ games per console!
            ...
            try:
                ...
                for h2 in soup.find_all('h2', class_='title'):
                    ...
                    if len(links) >= 3:
                        ...
                        if detail_url and zip_url.endswith('.zip'):
    ...
    def _get_zophar_tracks(self, pack_url: str) -> list:
        ...
        try:
            ...
            for a in soup.find_all('a', href=True):
                href = a.get('href')
                if 'zophar.zip' in href:
                    zip_links.append((a.text.strip(), href))
                    
            if zip_links:
                emu_link = None
                for text, href in zip_links:
                    if 'EMU' in href or 'original' in text.lower():
                        emu_link = href
                        break
                if not emu_link:
                    for text, href in zip_links:
                        if 'MP3' in href or 'mp3' in text.lower():
                            emu_link = href
                            break
                if not emu_link:
    ...
    def _search_vgmrips(self, query: str) -> list:
        ...
        if "/packs/pack/" in response.url:
            ...
            for a in soup.find_all('a'):
                href = a.get('href', '')
                if a.text.strip() == "Download" and href.endswith('.zip'):
                    if not href.startswith('http'):
                        href = "https://vgmrips.net" + href
                    zip_url = href
                    break
